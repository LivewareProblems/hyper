...
backend_t() ->
    ...
    ExpectedRegisters = lists:foldl(fun (Value, Registers) ->
                                    ...
                                    dict:new(),
                                    Values),
    ExpectedBytes = iolist_to_binary([begin
...
register_sum_t() ->
    ...
    ExpectedSum = math:pow(2, -0) * M - math:pow(2, -0) * length(SetRegisters) +
...
many_union_t() ->
    ...
    [begin
       ...
       ExpectedFilter = hyper:compact(hyper:insert_many(lists:flatten([sets:to_list(S)
                                                                       || S <- Sets]),
                                                        hyper:new(P, Mod))),
       ...
       case Delta < Card * NumSets * Error of
         true ->
             ok;
         false ->
             error_logger:info_msg("too high error, expected ~.2f%, actual ~.2f%~n~p, p = ~p, card "
                                   "= ~p",
...
union_t() ->
    ...
    ?assert(abs(hyper:card(UnionHyper) - sets:size(sets:union(LeftDistinct, RightDistinct))) <
              200),
    ?assert(abs(Intersection - sets:size(sets:intersection(LeftDistinct, RightDistinct))) <
              200).

union_mixed_precision_t() ->
    [?assertEqual(4,
                  trunc(hyper:card(hyper:union([hyper:insert(<<"1">>, hyper:new(4, Mod)),
...
bad_serialization_t() ->
    [begin
       ...
       Raw = case zlib:gunzip(base64:decode(binary:replace(WithNewlines, <<"\n">>, <<>>))) of
               <<RawBytes:M/binary>> ->
                   RawBytes;
               <<RawBytes:M/binary, 0>> ->
                   %% TODO: test padding
       ...
       H = hyper:from_json({[{<<"p">>, 15}, {<<"registers">>, base64:encode(zlib:gzip(Raw))}]},
                           Mod),
       ...
       ?assertEqual({[{<<"p">>, 15}, {<<"registers">>, base64:encode(zlib:gzip(Raw))}]},
                    hyper:to_json(H))
     end
     || Mod <- backends()].
...
prop_set() ->
    ?FORALL({Mod, P},
            ...
            ?FORALL(Values,
                    gen_getset(P),
                    begin
                      R = lists:foldl(fun ({Index, ZeroCount}, Register) ->
...
prop_serialize() ->
    ?FORALL({LeftMod, RightMod, P, Values},
